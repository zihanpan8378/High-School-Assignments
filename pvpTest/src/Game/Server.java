 ...
 */
import Game.MyPanel.Wall;
import static Game.MyPanel.obstacle;
import static Game.MyPanel.obstacleDynamic;
import static Game.MyPanel.wall;
...
public class Server extends JFrame {
                        ...
                        while ((read = bufferedReader.readLine()) != null) {
                            ...
                            for (int i = 0; i < read.length(); i++) {
                                if (read.substring(i, i + 1).equals(" ") && check == 0) {
                                    mp.x2 = Integer.parseInt(read.substring(0, i));
                                    read = read.substring(i + 1, read.length());
                                    i = 0;
                                    check++;
                                } else if (read.substring(i, i + 1).equals(" ") && check == 1) {
                                    mp.y2 = Integer.parseInt(read.substring(0, i));
                                    read = read.substring(i + 1, read.length());
                                    i = 0;
                                    check++;
                                } else if (read.substring(i, i + 1).equals(" ") && check == 2) {
                                    mp.dr2 = Integer.parseInt(read.substring(0, i));
                                    read = read.substring(i + 1, read.length());
                                    check++;
 ...
 /*try {
    ...
    public static void main(String[] args) throws IOException, SQLException {
        Server ss = new Server("192.168.100.3", 8888, "admin");
    }

    public Server(String ip, int port, String name) throws IOException, SQLException {
        // updateInfo();
        this.ip = ip;
        this.port = port;
                ...
                while (con) {
                    ...
                    if (check == 1) {
                        try {
                            ...
                            WiningPage a = new WiningPage(0, ip, port, name);
                            a.setVisible(true);
                            con = false;
                            setVisible(false);
                        ...
                        try {
                            ...
                            LosingPage a = new LosingPage(0, ip, port, name);
                            a.setVisible(true);
                            con = false;
                            setVisible(false);
                        } catch (SQLException ex) {
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < bullets.size(); i++) {
            ...
            if (!removeChecker(bullets.get(i).x, bullets.get(i).y)) {
                ...
                if ((bullets.get(i).x > x2 - 5 && bullets.get(i).x < x2 + 50) && (bullets.get(i).y > y2 - 5 && bullets.get(i).y < y2 + 50)) {
                    bullets.remove(i);
                    fireball.remove(i);
                    p2h -= 4;
                } else {
        ...
        for (int i = 0; i < bullets2.size(); i++) {
            ...
            if (!removeChecker(bullets2.get(i).x, bullets2.get(i).y)) {
                ...
                if ((bullets2.get(i).x > x - 5 && bullets2.get(i).x < x + 50) && (bullets2.get(i).y > y - 5 && bullets2.get(i).y < y + 50)) {
                    bullets2.remove(i);
                    fireball2.remove(i);
                    p1h -= 4;
                } else {
